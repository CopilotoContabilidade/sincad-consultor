    ...
    msg = client.messages.create(
        ...
        messages=[{
            ...
            "content": [
                ...
                {"type": "text", "text": "This is a CAPTCHA image. Reply with ONLY the characters you see — no spaces, no punctuation, no explanation."}
            ]
        }]
    )
        ...
        try:
            ...
            for inp in form.find_all('input'):
                ...
                if nm and nm != cnpj_fn and (
                    ...
                    'bdc' in fid or 'bdc' in fn or 'captchainput' in fcl
                ):
                    cap_fn = nm; break
            if not cap_fn:
                ti = [i for i in form.find_all('input') if i.get('type','text').lower() in ('text','')]
...
.pf{background:#472d54;height:10px;border-radius:8px;transition:width .3s}
...
<div class="main">
  <div class="card">
    ...
    <div class="fg">
      <div><label class="lbl">Nome da empresa *</label><input type="text" id="fn" placeholder="Ex: Tech Ltda"></div>
      <div><label class="lbl">CNPJ *</label><input type="text" id="fc" placeholder="00.000.000/0000-00" maxlength="18"></div>
      <div><label class="lbl">Insc. Estadual</label><input type="text" id="fi" placeholder="Opcional"></div>
...
function imp(e){
  ...
  r.onload=ev=>{try{
    ...
    if(!ns.length){toast('Nenhuma empresa válida');return}
...
function prog(f,t){document.getElementById('prog').innerHTML=`<div class="pb"><div class="pf" style="width:${Math.round(f/t*100)}%"></div></div><div class="pt">${f} de ${t} consultada(s)</div>`}
